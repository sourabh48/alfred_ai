...
class AwsDeployPipelineContractTests(SimpleTestCase):
    def test_github_actions_deploy_workflow_runs_ec2_pull_deploy(self):
        ...
        self.assertIn("secrets.AWS_EC2_HOST", workflow)
        self.assertIn("secrets.AWS_EC2_USER", workflow)
        self.assertIn("secrets.AWS_EC2_SSH_KEY", workflow)
        self.assertIn("ssh-keyscan", workflow)
        self.assertIn("git@github.com:${GITHUB_REPOSITORY}.git", workflow)
    ...
    def test_pipeline_docs_keep_delivery_separate_from_production_readiness(self):
        ...
        self.assertIn("read-only Deploy Key", docs)
