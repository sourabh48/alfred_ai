...
class InvestmentAndRelationshipAdvisoryTests(TestCase):
    ...
    def test_recommendation_overview_suppresses_credit_cards_and_personal_loans(self):
        ...
        self.assertEqual(payload["grounding"]["freshness"]["tracked_records"], 2)
        self.assertTrue(payload["investments"]["grounding"]["proof_contract"]["complete"])
        self.assertTrue(payload["insurance"]["grounding"]["proof_contract"]["complete"])
        self.assertEqual(payload["credit_cards"]["grounding"]["proof_contract"]["required_sources"], ["World Bank"])
        self.assertTrue(payload["credit_cards"]["grounding"]["proof_contract"]["complete"])
        self.assertTrue(payload["loans"]["grounding"]["proof_contract"]["complete"])
        self.assertTrue(payload["grounding"]["proof_contract"]["complete"])
        self.assertEqual(payload["grounding"]["proof_contract"]["refresh_contract"]["scheduled_refresh"], "refresh_due_records")
