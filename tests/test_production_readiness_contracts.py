...
class ProductionReadinessContractTests(SimpleTestCase):
    def test_readme_and_production_doc_keep_production_maturity_gated(self):
        ...
        self.assertIn("AWS Free Tier Only Deployment", readme)
        self.assertIn("AWS Deployment Guide", readme)
        self.assertIn("docker-compose.aws-local.yml", readme)
        self.assertIn("docker-compose.aws-free-tier.yml", readme)
        ...
        self.assertIn("AWS Free Tier Only Deployment", production)
        ...
        self.assertIn("Recommended Free Tier Only Shape", aws)
        self.assertIn("Docker PostgreSQL on EC2", aws)
        self.assertIn("Docker on EC2", aws)
        self.assertIn("Do not create RDS or ElastiCache", aws)
        self.assertIn("opt-in only after the AWS Billing console proves they are covered", aws)
        self.assertIn("Blocker-To-Proof Checklist", aws)
        self.assertIn("Do not raise Deployment Readiness to 100%", aws)

        free_tier = (REPO_ROOT / "docs" / "AWS_FREE_TIER_ONLY.md").read_text(encoding="utf-8")
        self.assertIn("strict AWS Free Tier path", free_tier)
        self.assertIn("docker-compose.aws-free-tier.yml", free_tier)
        self.assertIn("Do not create RDS, ElastiCache", free_tier)
        self.assertIn("one EC2 host", free_tier)
    ...
    def test_aws_env_and_docker_contracts_cover_web_postgres_redis_and_celery(self):
        ...
        free_tier_compose = (REPO_ROOT / "docker-compose.aws-free-tier.yml").read_text(encoding="utf-8")
        aws_env = (REPO_ROOT / "config" / "aws.env.example").read_text(encoding="utf-8")
        free_tier_env = (REPO_ROOT / "config" / "aws-free-tier.env.example").read_text(encoding="utf-8")
        ...
        for service in ("postgres:", "redis:", "web:", "worker:", "beat:"):
            with self.subTest(service=service):
                ...
                self.assertIn(service, free_tier_compose)

        self.assertIn("python manage.py migrate", compose)
        self.assertIn("python manage.py migrate", free_tier_compose)
        self.assertIn("python manage.py collectstatic --noinput", compose)
        self.assertIn("python manage.py collectstatic --noinput", free_tier_compose)
        ...
        self.assertIn("DEBUG=false", free_tier_env)
        self.assertIn("SECURE_SSL_REDIRECT=true", aws_env)
        self.assertIn("SECURE_SSL_REDIRECT=true", free_tier_env)
        self.assertIn("SESSION_COOKIE_SECURE=true", aws_env)
        self.assertIn("SESSION_COOKIE_SECURE=true", free_tier_env)
        self.assertIn("CSRF_COOKIE_SECURE=true", aws_env)
        self.assertIn("CSRF_COOKIE_SECURE=true", free_tier_env)
        self.assertIn("SECURE_PROXY_SSL_HEADER_ENABLED=true", free_tier_env)
        self.assertIn("ALFRED_PRODUCTION_DEPLOYMENT_PROOF", aws_env)
        self.assertIn("ALFRED_PRODUCTION_DEPLOYMENT_PROOF", free_tier_env)
        self.assertIn("ALFRED_MATERIALIZED_CACHE_TRAFFIC_PROOF", aws_env)
        self.assertIn("ALFRED_MATERIALIZED_CACHE_TRAFFIC_PROOF", free_tier_env)
        self.assertIn("ALFRED_DELETE_SOURCE_UPLOADS_AFTER_EXTRACTION=true", aws_env)
        self.assertIn("ALFRED_DELETE_SOURCE_UPLOADS_AFTER_EXTRACTION=true", free_tier_env)
