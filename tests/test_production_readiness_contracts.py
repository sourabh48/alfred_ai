...
class ProductionReadinessContractTests(SimpleTestCase):
    def test_readme_and_production_doc_keep_local_hosting_default_and_public_maturity_gated(self):
        readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
        production = (REPO_ROOT / "docs" / "PRODUCTION_READINESS.md").read_text(encoding="utf-8")
        local = (REPO_ROOT / "docs" / "LOCAL_HOSTING.md").read_text(encoding="utf-8")
        ...
        self.assertIn("Local Hosting", readme)
        self.assertIn("ALFRED is local or LAN-hosted", readme)
        self.assertIn("docker-compose.local.yml", readme)
        self.assertIn("Data Privacy And Storage", readme)
        self.assertIn("ALFRED_DELETE_SOURCE_UPLOADS_AFTER_EXTRACTION=true", readme)
        self.assertIn("local or LAN-hosted application by default", production)
        self.assertIn("ALFRED should not be exposed to the public internet", production)
        self.assertIn("Celery worker and beat", production)
        self.assertIn("local defaults and `docker-compose.local.yml`", production)
        self.assertIn("sustained production-like cache telemetry", production)
        self.assertIn("Required Environment Variables For Public Production", production)
    ...
    def test_local_env_and_docker_contracts_cover_web_postgres_redis_and_celery(self):
        ...
        local_env = (REPO_ROOT / "config" / "local.env.example").read_text(encoding="utf-8")
        ...
        self.assertIn("${ALFRED_LOCAL_ENV_FILE:-config/local.env}", compose)
        self.assertIn("${ALFRED_WEB_PORT:-8000}:8000", compose)
        self.assertIn("alfred_local_postgres", compose)
        self.assertIn("alfred_local_redis", compose)
        self.assertIn("alfred_local_media", compose)
        self.assertIn("alfred_local_artifacts", compose)
        self.assertIn("django.core.cache.backends.redis.RedisCache", local_env)
        self.assertIn("DB_ENGINE=django.db.backends.postgresql", local_env)
        self.assertIn("DB_HOST=postgres", local_env)
        self.assertIn("DB_CONN_MAX_AGE=60", local_env)
        self.assertIn("CACHE_LOCATION=redis://redis:6379/1", local_env)
        self.assertIn("REDIS_URL=redis://redis:6379/0", local_env)
        self.assertIn("CELERY_BROKER_URL=redis://redis:6379/0", local_env)
        self.assertIn("CELERY_RESULT_BACKEND=redis://redis:6379/0", local_env)
        self.assertIn("CELERY_TASK_ALWAYS_EAGER=false", local_env)
        self.assertIn("ALFRED_LOCAL_RUNTIME=false", local_env)
        self.assertIn("DEBUG=false", local_env)
        self.assertIn("SECURE_SSL_REDIRECT=false", local_env)
        self.assertIn("SESSION_COOKIE_SECURE=false", local_env)
        self.assertIn("CSRF_COOKIE_SECURE=false", local_env)
        self.assertIn("ALFRED_PRODUCTION_DEPLOYMENT_PROOF=artifacts/ops/local_readiness_summary.json", local_env)
        self.assertIn("ALFRED_MATERIALIZED_CACHE_TRAFFIC_PROOF", local_env)
        self.assertIn("ALFRED_DELETE_SOURCE_UPLOADS_AFTER_EXTRACTION=true", local_env)
