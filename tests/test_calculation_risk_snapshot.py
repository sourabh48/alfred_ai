...
class CalculationRiskSnapshotTests(TestCase):
    ...
    def test_loan_repayment_review_gate_and_cash_flow_aggregation_are_reported(self):
        ...
        self.assertEqual(verified["canonical_financial_baseline"]["status"], "verified_formula_review_gated_data")
        self.assertIn("metric_states", " ".join(verified["canonical_financial_baseline"]["evidence"]))
        self.assertEqual(snapshot["critical_path_progress"]["financial_baseline"], 80)
